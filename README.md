...
Alternatively, build the full course (lessons, labs, and course-wide materials) by using `build.sh` in the root of the repository:
...
```bash
$ pyflakes lecture/*.py course_wide/*.py labs/*.py
$ pycodestyle lecture/*.py course_wide/*.py labs/*.py
$ npx eslint 'lecture/**/*.js'
```
