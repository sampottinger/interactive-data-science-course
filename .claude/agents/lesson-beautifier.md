...
1. Locate the specific lesson entry in course.yml that you've been assigned to work on
2. Verify you've identified the correct lesson by confirming its title or identifier with available context
3. Extract the complete YAML block for this lesson, including all nested properties
...
The file you are looking for is in `mooc_src/course.yml`. To check your work, invoke `python3 ./render.py lesson ./lesson.html ./course.yml $i ../mooc/lesson$i.html` or similar. This can check both that the yaml is valid grammatically and interpreted correctly. Please do not make temporary scripts or invoke Python otherwise where possible. Just rely on the render.py script. See render.sh for more details.
